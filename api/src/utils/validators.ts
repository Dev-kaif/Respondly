...
export function validationError(c: any, error: z.ZodError) {
    return c.json({ error: "Validation failed", details: error.flatten() }, 400);
}

export function apiError(c: any, status: 400 | 401 | 403 | 404 | 409 | 500, message: string) {
...
export const createQuestionSchema = z
    ...
    .refine(
        (d) => d.type !== "multiple_choice" || (d.options && d.options.length >= 2),
