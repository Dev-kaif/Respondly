...
router.post("/survey/:slug/submit", async (c) => {
    ...
    const missingRequired = requiredQuestions.find(
        (q) => !answeredIds.has(q.id) || !body.answers.find((a) => a.questionId === q.id && a.value.trim() !== "")
    );
...
router.get("/forms/:id/export", async (c) => {
    ...
    const answerMap = new Map(
        allAnswers.map((a) => [
            `${a.responses.id}:${a.answers.questionId}`,
            a.answers.value,
        ])
    );
    ...
    const records = allResponses.map((response) =>
        formQuestions.map(
            (question) => answerMap.get(`${response.id}:${question.id}`) ?? ""
        )
    );
